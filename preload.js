...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  setBookmarkBarVisibility: (visible) =>
    ipcRenderer.send("set-bookmark-bar-visibility", visible),
  ...
  notify: (message, type = "info", duration = 3000) =>
    ipcRenderer.send("notify", { message, type, duration }),
  ...
  setDownloadLocation: (path) =>
    ipcRenderer.invoke("set-download-location", path),
  applyUISettings: (settings) =>
    ipcRenderer.invoke("apply-ui-settings", settings),
  applyWebDarkMode: (viewId, enabled) =>
    ipcRenderer.invoke("apply-web-dark-mode", viewId, enabled),
  applyWebDarkModeAll: (enabled) =>
    ipcRenderer.invoke("apply-web-dark-mode-all", enabled),
  setZoomLevel: (zoom) => ipcRenderer.invoke("set-zoom-level", zoom),
  setCloseTabsOnExit: (enabled) =>
    ipcRenderer.invoke("set-close-tabs-on-exit", enabled),
  setTabPreviewsEnabled: (enabled) =>
    ipcRenderer.invoke("set-tab-previews-enabled", enabled),
  ...
  checkDropTarget: (screenX, screenY, tabMeta) =>
    ipcRenderer.invoke("check-drop-target", { screenX, screenY, tabMeta }),
  ...
  startWindowDrag: (payload = {}) =>
    ipcRenderer.send("start-window-drag", payload),
  ...
  onNewWindow: (callback) =>
    ipcRenderer.on("new-window", (event, url) => callback(url)),
  onThemeChanged: (callback) =>
    ipcRenderer.on("theme-changed", (_event, theme) => callback(theme)),
  onWidgetSettingsChanged: (callback) =>
    ipcRenderer.on("widget-settings-changed", (_event, data) => callback(data)),
  broadcastThemeChange: (theme) =>
    ipcRenderer.send("broadcast-theme-change", theme),
  ...
  onAutoUpdaterDebugInfo: (callback) =>
    ipcRenderer.on("auto-updater-debug-info", (_event, debugInfo) =>
      callback(debugInfo),
    ),
...
function renderLocalToast({ message, type = "info", duration = 3000 } = {}) {
  try {
    ...
    console.error("Failed to render local toast", e);
  }
}
